    ...
    if (tabs) {
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                // Добавление активного класса к нажатой вкладке
                tab.classList.add('active');
                
                // Скрытие всех списков
                [actressList, achievementsList, allRolesList].forEach(list => {
                    if (list) list.classList.add('hidden');
                });

                // Показ соответствующего списка                const text = tab.textContent.trim();
                if (text === 'Актриса') {
                    actressList.classList.remove('hidden');
                } else if (text === 'Режиссер') {
                    document.getElementById('director-list').classList.remove('hidden');
                } else if (text === 'Все роли') {
                    allRolesList.classList.remove('hidden');
                }
            });
        });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    // Получаем все вкладки и списки
    const tabs = document.querySelectorAll('.tab');
    const lists = {
        'Актриса': document.getElementById('actress-list'),
        'Режиссер': document.getElementById('director-list'),
        'Все роли': document.getElementById('all-roles-list')
    };
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', function() {
            ...
            const listToShow = lists[this.textContent.trim()];
            if (listToShow) {
